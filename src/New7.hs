...
t =
  parseStatements
    ...
    "hello SQL"
    Nothing
    "SELECT sum(*) FROM (SELECT sum(a.value) From Person where a.value+1 > 0 GROUP BY TIMEWINDOW(5)) GROUP BY TIMEWINDOW(5) "

-- "SELECT sum(c + a) + a  FROM Person GROUP BY TIMEWINDOW(5) "
...
sum0 :: Int -> Tdynamic -> (Int, Tdynamic)
sum0 s m = (s, m)

type Fun state = state -> Tdynamic -> IO (state, Tdynamic)

conn :: Fun a -> Fun b -> Fun (a, b)
conn = undefined
